...
var JpxImage = (function JpxImageClosure() {
					...
					if (inclusionTree.reset(codeblockColumn, codeblockRow)) {
						while (true) {
							if (readBits(1)) {
								...
								inclusionTree.incrementValue(0);
								break;
							}
						}
					}
	...
	function transformComponents(context) {
		...
		if (isSigned) {
			...
			shift = component.precision - 8;
			offset = (128 << shift) + 0.5;
		}

		for (pos = 0, j = 0, jj = items.length; j < jj; j++) {
			val = items[j];
			out[pos] = (val + offset);
			pos += 1;
		}
	...
	function initializeTile(context) {
		var tile = context.tile;
		var component = tile.component;
		component.quantizationParameters = context.currentTile.QCD;
		component.codingStyleParameters = context.currentTile.COD;
